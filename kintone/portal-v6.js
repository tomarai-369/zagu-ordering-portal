(function () {
  ...
  var APPS = {
    products: { id: 1, label: "Products Master", icon: "📦", color: "#E8740C", desc: "Catalog, pricing & stock" },
    dealers: { id: 2, label: "Dealers Master", icon: "🏪", color: "#1a7fa8", desc: "Accounts, regions & credit" },
    orders: { id: 3, label: "Orders", icon: "📋", color: "#8b5cf6", desc: "Processing & fulfillment" },
    news: { id: 4, label: "Announcements", icon: "📣", color: "#d97706", desc: "News, promos & updates" },
    holidays: { id: 5, label: "Holiday Calendar", icon: "📅", color: "#059669", desc: "PH holidays & business days" }
  };
  ...
  var PEAK_MONTHS = [3, 4, 5, 6, 10, 11, 12]; // Mar-Jun, Oct-Dec
  ...
  var CSS = `
    ...
    .zp-greeting { position: absolute; top: 20px; right: 28px; z-index: 1; font-size: 12px; opacity: 0.85; text-align: right; }
    .zp-greeting strong { font-weight: 600; }
    .zp-greeting-date { display: block; font-size: 11px; opacity: 0.7; margin-top: 2px; }
    ...
    .zp-peak-icon { font-size: 16px; flex-shrink: 0; }
    .zp-peak-text strong { font-weight: 700; }
    .zp-peak-dismiss {
      margin-left: auto; cursor: pointer; opacity: 0.5; font-size: 14px; 
      background: none; border: none; color: #92400e; padding: 2px 6px;
    }
    .zp-peak-dismiss:hover { opacity: 1; }

    /* KPIs */
    .zp-kpis { display: grid; grid-template-columns: repeat(5, 1fr); gap: 12px; margin-bottom: 24px; }
    .zp-kpi {
      background: var(--zp-surface); border-radius: var(--zp-radius);
      padding: 18px 20px; box-shadow: var(--zp-shadow-sm);
    ...
    .zp-kpi-val { font-size: 28px; font-weight: 700; line-height: 1.2; letter-spacing: -1px; margin-top: 8px; }
    .zp-kpi-val.zp-kpi-val-sm { font-size: 20px; letter-spacing: -0.5px; }
    .zp-kpi-lbl { font-size: 10.5px; color: var(--zp-text-secondary); text-transform: uppercase; letter-spacing: 0.8px; font-weight: 500; }
    .zp-kpi-alert { display: inline-block; font-size: 10px; font-weight: 600; padding: 2px 8px; border-radius: 99px; margin-left: 6px; vertical-align: middle; }
    .zp-kpi-trend { font-size: 11px; color: var(--zp-text-muted); margin-top: 4px; font-weight: 500; }
    .zp-kpi-trend-up { color: #059669; }
    .zp-kpi-trend-down { color: #dc2626; }
    ...
    /* Sparkline */
    .zp-spark { position: absolute; bottom: 0; right: 0; width: 80px; height: 36px; opacity: 0.15; }
    ...
    /* Apps */
    .zp-apps { display: grid; grid-template-columns: repeat(5, 1fr); gap: 12px; margin-bottom: 24px; }
    .zp-app {
      background: var(--zp-surface); border-radius: var(--zp-radius);
      padding: 18px 16px; box-shadow: var(--zp-shadow-sm);
      border: 1px solid var(--zp-border);
    ...
    .zp-app-icon { width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 18px; }
    .zp-app-name { font-size: 13px; font-weight: 600; }
    .zp-app-desc { font-size: 11.5px; color: var(--zp-text-secondary); line-height: 1.4; }
    ...
    .zp-wf-wrap {
      ...
      border: 1px solid var(--zp-border); margin-bottom: 24px;
      overflow-x: auto;
    }
    ...
    .zp-modal-total { font-size: 12px; color: #059669; font-weight: 600; margin-left: 4px; }
    .zp-modal-close {
    ...
    .zp-modal-body { overflow-y: auto; flex: 1; padding: 0; }
    .zp-modal-loading { padding: 48px; text-align: center; color: #a8a29e; font-size: 13px; }
    .zp-modal-empty { padding: 48px; text-align: center; color: #a8a29e; font-size: 14px; }

    /* Modal Table */
    .zp-dt { width: 100%; border-collapse: collapse; font-size: 13px; }
    .zp-dt thead { position: sticky; top: 0; z-index: 1; }
    .zp-dt th {
      background: #fafaf9; padding: 10px 14px; text-align: left;
    ...
    .zp-dt .zp-dt-region { font-size: 11px; color: #78716c; }
    ...
    /* Announcements */
    .zp-ann { margin-bottom: 24px; }
    .zp-ann-item {
      background: var(--zp-surface); border-radius: 10px;
      padding: 11px 16px; box-shadow: var(--zp-shadow-sm);
    ...
    /* Quick Actions */
    .zp-qa { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 24px; }
    .zp-qa-item {
      background: var(--zp-surface); border-radius: 10px;
    ...
    @media (max-width: 960px) {
      .zp-kpis { grid-template-columns: repeat(3, 1fr); }
      .zp-apps { grid-template-columns: repeat(3, 1fr); }
      .zp-qa { grid-template-columns: repeat(2, 1fr); }
      .zp-greeting { display: none; }
      .zp-modal { width: 96%; max-height: 85vh; }
    }
    @media (max-width: 600px) {
      .zp-kpis { grid-template-columns: repeat(2, 1fr); }
      .zp-apps, .zp-qa { grid-template-columns: 1fr; }
      #zagu-portal { padding: 12px 12px 32px; }
      .zp-kpi-val { font-size: 24px; }
      .zp-kpi-val.zp-kpi-val-sm { font-size: 17px; }
    }
  `;
  ...
  function fmtDateFull() {
    ...
    return days[d.getDay()] + ", " + m[d.getMonth()] + " " + d.getDate() + ", " + d.getFullYear();
  }
  ...
  function animateValue(el, start, end, duration, prefix, suffix) {
    ...
    suffix = suffix || "";
    var range = end - start;
    if (range === 0) { el.textContent = prefix + end + suffix; return; }
    var startTime = null;
    function step(ts) {
      ...
      '<div class="zp-modal">' +
        '<div class="zp-modal-header">' +
          '<div class="zp-modal-header-left">' +
            ...
            '<span class="zp-modal-total" data-modal-total></span>' +
          '</div>' +
          '<button class="zp-modal-close" data-modal-close>✕</button>' +
        '</div>' +
        '<div class="zp-modal-body" data-modal-body>' +
          '<div class="zp-modal-loading">Loading orders...</div>' +
        '</div>' +
        '<div class="zp-modal-footer">' +
          '<a href="' + DOMAIN + '/k/3/" data-modal-link>Open in Orders app →</a>' +
  ...
  function openModal(node) {
    ...
    var totalEl = overlay.querySelector("[data-modal-total]");
    ...
    fetchRecords(3, q, fields).then(function (records) {
      ...
      var sumTotal = 0;
      records.forEach(function (r) { sumTotal += parseFloat(r.total_amount.value) || 0; });
      if (sumTotal > 0) totalEl.textContent = "· " + fmtPeso(sumTotal);
      ...
      var html = '<table class="zp-dt"><thead><tr>' +
        '<th>Order #</th><th>Date</th><th>Dealer</th><th>Region</th><th>Amount</th>';
      ...
      records.forEach(function (r) {
        ...
        var dealer = r.dealer_name_display.value || "—";
        var region = r.dealer_region_display.value || "—";
        var amount = fmtPeso(r.total_amount.value);
        var recUrl = DOMAIN + "/k/3/show#record=" + recId;

        var extraCol;
        if (isRejected) {
          extraCol = '<td style="font-size:12px;color:#dc2626">' + (r.rejection_reason.value || "—") + '</td>';
        } else if (isSAP) {
          var sapNo = r.sap_sales_order_no.value;
          extraCol = '<td>' + (sapNo ? '<span style="font-weight:600;color:#8b5cf6">' + sapNo + '</span>' : '—') + '</td>';
        } else {
          var ps = r.payment_status.value || "—";
        ...
        html += '<tr onclick="window.location.href=\'' + recUrl + '\'">' +
          ...
          '<td>' + date + '</td>' +
          '<td>' + dealer + '</td>' +
          '<td class="zp-dt-region">' + region + '</td>' +
  ...
  function closeModal() {
    if (modal) modal.classList.remove("zp-modal-open");
  }
  ...
  function buildPeakBanner() {
    var month = new Date().getMonth() + 1;
    if (PEAK_MONTHS.indexOf(month) === -1) return null;

    var el = document.createElement("div");
    el.className = "zp-peak";
    el.innerHTML =
      '<span class="zp-peak-icon">🔥</span>' +
      '<span class="zp-peak-text"><strong>Peak Season Active</strong> — Higher order volumes expected. Ensure timely approvals and fulfillment.</span>' +
      '<button class="zp-peak-dismiss" title="Dismiss">✕</button>';
    el.querySelector(".zp-peak-dismiss").addEventListener("click", function () { el.remove(); });
    return el;
  }
  ...
  function buildKPIs() {
    ...
    var items = [
      ...
      { key: "revenue", label: "This Month Revenue", color: "#059669", isRevenue: true }
    ];

    items.forEach(function (kpi) {
      var card = document.createElement("a");
      card.className = "zp-kpi";
      if (kpi.view) card.href = DOMAIN + "/k/" + kpi.app + "/?view=" + kpi.view;
      else if (kpi.isRevenue) card.href = DOMAIN + "/k/3/";
      card.innerHTML =
        '<div class="zp-kpi-lbl"><span class="zp-kpi-dot" style="background:' + kpi.color + '"></span> ' + kpi.label + '</div>' +
        '<div class="zp-kpi-val' + (kpi.isRevenue ? ' zp-kpi-val-sm' : '') + ' zp-loading" data-k="' + kpi.key + '">—</div>' +
        (kpi.isRevenue ? '<div class="zp-kpi-trend" data-kt="' + kpi.key + '"></div>' : '');
      el.appendChild(card);

      if (kpi.isRevenue) {
        // Calculate month revenue from orders
        var now = new Date();
        ...
        // Current month
        var curQ = 'order_date >= "' + ym + '-01" and Status not in ("Rejected", "New") limit 500';
        var lastQ = 'order_date >= "' + lastMonth + '-01" and order_date < "' + ym + '-01" and Status not in ("Rejected", "New") limit 500';

        Promise.all([
          fetchRecords(3, curQ, ["total_amount"]),
          fetchRecords(3, lastQ, ["total_amount"])
        ]).then(function (results) {
          var curRev = 0, lastRev = 0;
          results[0].forEach(function (r) { curRev += parseFloat(r.total_amount.value) || 0; });
          results[1].forEach(function (r) { lastRev += parseFloat(r.total_amount.value) || 0; });

          var v = card.querySelector("[data-k='revenue']");
          v.textContent = fmtPesoShort(curRev);
          v.classList.remove("zp-loading");
          v.style.color = kpi.color;

          // Trend vs last month
          var trend = card.querySelector("[data-kt='revenue']");
          if (lastRev > 0) {
            var pct = Math.round(((curRev - lastRev) / lastRev) * 100);
            var arrow = pct >= 0 ? "↑" : "↓";
            trend.className = "zp-kpi-trend " + (pct >= 0 ? "zp-kpi-trend-up" : "zp-kpi-trend-down");
            trend.textContent = arrow + " " + Math.abs(pct) + "% vs last month";
          } else {
            trend.textContent = fmtPeso(curRev) + " total";
            trend.className = "zp-kpi-trend";
          }
        }).catch(function () {});
      } else {
  ...
  function buildApps() {
    ...
    Object.keys(APPS).forEach(function (key) {
      ...
      card.innerHTML =
        ...
        '<div class="zp-app-meta" data-ac="' + app.id + '">—</div>';
      el.appendChild(card);
  ...
  function buildAnnouncements() {
    ...
    fetchRecords(4, 'is_active in ("Yes") order by is_pinned desc, priority asc limit 5',
      ...
      records.forEach(function (r) {
        ...
        item.innerHTML =
          (pinned ? '<span class="zp-ann-pin">📌</span>' : '') +
      ...
      more.textContent = "View all announcements →";
  ...
  function buildQuickActions() {
    ...
    el.className = "zp-qa";
    var actions = [
      { icon: "📋", bg: "#fef3c7", text: "Pending My Approval", sub: "Awaiting your action", href: "/k/3/?view=%F0%9F%93%8B%20Pending%20My%20Approval" },
      { icon: "💳", bg: "#fce7f3", text: "Unpaid Orders", sub: "Payment follow-ups", href: "/k/3/?view=%F0%9F%92%B3%20Unpaid%20Orders" },
      { icon: "📦", bg: "#dbeafe", text: "Warehouse Pipeline", sub: "Picking → Ready → Done", href: "/k/3/?view=%F0%9F%93%A6%20Warehouse%20Pipeline" },
      { icon: "🟢", bg: "#d1fae5", text: "Active Products", sub: "Full catalog", href: "/k/1/?view=%F0%9F%9F%A2%20Active%20Products" },
      { icon: "⚠️", bg: "#fef9c3", text: "Low Stock Items", sub: "≤ 50 units left", href: "/k/1/?view=%E2%9A%A0%EF%B8%8F%20Low%20Stock%20(%E2%89%A450)" },
      { icon: "💰", bg: "#fee2e2", text: "Outstanding Balances", sub: "Dealers with balances", href: "/k/2/?view=%E2%9A%A0%EF%B8%8F%20Outstanding%20Balances" },
      { icon: "📅", bg: "#d1fae5", text: "Holiday Calendar", sub: "2026 PH holidays", href: "/k/5/?view=%F0%9F%93%85%20All%20Holidays" },
      { icon: "📖", bg: "#ede9fe", text: "Dealer User Guide", sub: "Ordering manual", href: "/k/4/?view=%F0%9F%93%96%20Guides" }
    ];
  ...
  kintone.events.on("portal.show", function () {
    ...
    hdr.innerHTML =
      ...
      '<div class="zp-greeting">' + greet() + ', <strong>' + (user.name || "User") + '</strong>' +
        '<span class="zp-greeting-date">' + fmtDateFull() + '</span>' +
      '</div>';
    root.appendChild(hdr);

    // Peak Season Banner
    var peak = buildPeakBanner();
    if (peak) root.appendChild(peak);
    ...
    ft.innerHTML =
      '<span>Zagu Ordering System · Powered by Kintone · Built by Edamame Inc.</span>' +
      '<span>Updated ' + fmtTime() + ' &nbsp;<button class="zp-refresh" onclick="location.reload()">↻ Refresh</button></span>';
    root.appendChild(ft);
